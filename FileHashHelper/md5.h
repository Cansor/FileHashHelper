...
public:
	...
	const byte* digest();
	string toString(bool upper = false);
    std::wstring toStringW(bool upper = false);
	void reset();
...
private:
	...
	//string bytesToHexString(const byte* input, size_t length, bool upper);
    //std::wstring bytesToHexWString(const byte* input, size_t length, bool upper);
