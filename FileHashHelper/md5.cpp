/*
 * 这份代码来源于 #ant - C++ 博客，并在原有基础上修改了部分代码。
 * 原文链接：
...
MD5::MD5(const void* input, size_t length):MD5() {
	update(input, length);
}

/* Construct a MD5 object with a string. */
MD5::MD5(const string& str) : MD5() {
    update(str);
}

/* Construct a MD5 object with a file. */
MD5::MD5(ifstream& in):MD5() {
	update(in);
}
...
//string MD5::bytesToHexString(const byte* input, size_t length, bool upper) {
//	string str;
...
//wstring MD5::bytesToHexWString(const byte* input, size_t length, bool upper) {
//    wstring str;
//    str.reserve(length << 1);
//    for (size_t i = 0; i < length; ++i) {
//		int t = input[i];
...
string MD5::toString(bool upper) {
...
wstring MD5::toStringW(bool upper) {
    constexpr int32_t length = 16;
    const byte* input = digest();
    wstring str;
