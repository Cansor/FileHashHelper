...
/*
 * 在原有基础上修改了部分代码。
 * 原始代码请查阅：https://github.com/vog/sha1
 */
...
public:
    ...
    void update(const char* bytes, std::streamsize length);
    std::string final(bool upper = false);
    std::wstring finalW(bool upper = false);
...
inline void SHA1::update(const char* bytes, std::streamsize length)
{
    uint_fast8_t j = 0;
    char sbuf[BLOCK_BYTES];
...
inline std::string SHA1::final(bool upper)
{
    end();
...
std::wstring SHA1::finalW(bool upper) {
    end();
    ...
    if (upper) result << std::uppercase;
    for (size_t i = 0; i < sizeof(digest) / sizeof(digest[0]); i++) {
        result << std::hex << std::setfill(L'0') << std::setw(8);
        result << digest[i];
    }
